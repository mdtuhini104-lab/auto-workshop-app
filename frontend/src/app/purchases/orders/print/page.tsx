...
function PurchaseOrderPrintPageContent() {
  ...
  return (
    <div className="min-h-screen bg-white p-8 flex flex-col items-center">
      ...
      <div id="printable-po-doc" className="w-full max-w-[210mm] bg-white border border-slate-300 p-8 rounded-lg space-y-6">
        <style dangerouslySetInnerHTML={{__html: `
          @media print {
            body * { visibility: hidden; }
            #printable-po-doc, #printable-po-doc * { visibility: visible; }
            #printable-po-doc { position: absolute; left: 0; top: 0; width: 100%; border: none; }
          }
        `}} />
        ...
        <div className="border-b border-slate-300 pb-4 text-center">
          ...
          <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider">Purchase Order Voucher</p>
        </div>

        <div className="flex justify-between items-center text-xs">
          <div>
            <p className="font-bold text-base text-slate-900">PO Number: {order.id}</p>
            <p className="text-slate-600">Vendor: {order.vendorName}</p>
          </div>
          <div className="text-right">
            <p className="text-slate-600">Order Date: {order.orderDate}</p>
            <p className="text-slate-600">Expected: {order.expectedDate}</p>
          </div>
        </div>

        <div className="border border-slate-200 rounded p-4 text-center space-y-1 bg-slate-50">
          <p className="text-xs text-slate-500 font-bold uppercase">Total Order Amount</p>
          <p className="text-3xl font-black font-mono text-[#004e89]">৳ {order.totalAmount.toLocaleString('en-BD')}</p>
        </div>
        ...
        <div className="flex justify-between pt-16 text-center text-xs font-semibold text-slate-700">
          ...
          <div className="w-48 border-t border-dashed border-slate-400 pt-2">Authorized Officer</div>
        </div>
      </div>
    </div>
  );
